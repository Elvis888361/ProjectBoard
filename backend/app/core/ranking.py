"""Generates sortable task positions for efficient drag-and-drop task ordering."""
...
class InvalidPosition(ValueError):
    # Raised when task position key is invalid or incorrectly formatted.
    pass


def _integer_len(head: str) -> int:
    """Determines integer section length from position key's first character."""
...
def _integer_part(key: str) -> str:
    """Extracts integer portion from a complete position key."""
...
def _validate(key: str) -> None:
    """Validates position key format and prevents invalid ordering values."""
...
def _increment_integer(x: str) -> str | None:
    """Generates next valid integer position for task ordering."""
...
def _decrement_integer(x: str) -> str | None:
    """Generates previous valid integer position for task ordering."""
...
def _midpoint(a: str, b: str | None) -> str:
    """Creates position key exactly between two existing task positions."""
...
def key_between(a: str | None, b: str | None) -> str:
    """Generates sortable position between, before, or after existing task keys."""
    if a is not None:
        _validate(a)
    if b is not None:
        _validate(b)
